...
#include "systems/trajectory_optimization/dircol/dircol_position_data.h"
#include "systems/trajectory_optimization/dircol/dircol_distance_data.h"
...
#include "systems/trajectory_optimization/dircol/hybrid_dircol.h"
...
using dairlib::systems::trajectory_optimization::HybridDircol;
using dairlib::systems::trajectory_optimization::DircolDynamicConstraint;
using dairlib::systems::trajectory_optimization::DircolKinematicConstraint;
using dairlib::systems::trajectory_optimization::DircolOptions;
using dairlib::systems::trajectory_optimization::DircolKinConstraintType;
using dairlib::systems::trajectory_optimization::DircolAbstractConstraint;
...
/// constraints in trajectory optimization (dircol).
...
void GetInitGuessForUAndLambda(const MultibodyPlant<double>& plant,
                               DircolKinematicDataSet<double>& left_dataset,
  ...
  for (unsigned int i = 0; i < q_seed.size(); i++) {
    ...
    for (int j = 0; j < left_dataset.getNumConstraintObjects(); j++) {
      DircolKinematicData<double>* cj = left_dataset.getConstraint(j);
...
class QuaternionNormConstraint : public DircolAbstractConstraint<double> {
 public:
  QuaternionNormConstraint() : DircolAbstractConstraint<double>(1, 4,
...
void DoMain(double stride_length, double duration_ss, int iter,
  ...
  // Set up contact/distance constraints and construct dircol
  // parameters
  ...
  Vector3d pt_rear_contact(0.088, 0, 0);
  bool isXZ = false;
  auto left_toe_front_constraint = DircolPositionData<double>(plant, toe_left,
                                   pt_front_contact, isXZ);
  auto left_toe_rear_constraint = DircolPositionData<double>(plant, toe_left,
                                  pt_rear_contact, isXZ);
  auto right_toe_front_constraint = DircolPositionData<double>(plant, toe_right,
                                    pt_front_contact, isXZ);
  auto right_toe_rear_constraint = DircolPositionData<double>(plant, toe_right,
                                   pt_rear_contact, isXZ);
  ...
  auto distance_constraint_left = DircolDistanceData<double>(plant,
                                  thigh_left, pt_on_thigh_left,
                                  heel_spring_left, pt_on_heel_spring,
                                  rod_length);
  auto distance_constraint_right = DircolDistanceData<double>(plant,
                                   thigh_right, pt_on_thigh_right,
                                   heel_spring_right, pt_on_heel_spring,
                                   rod_length);

  // Testing: only four bar linkage constraint
  vector<DircolKinematicData<double>*> no_stance_constraint;
  // no_stance_constraint.push_back(&distance_constraint_left);
  // no_stance_constraint.push_back(&distance_constraint_right);
  auto no_stance_dataset = DircolKinematicDataSet<double>(plant,
                      &no_stance_constraint);
  auto no_stance_options = DircolOptions(no_stance_dataset.countConstraints());

  // Testing
  bool isYZ = true; if (isYZ) isXZ = true;
  auto left_toe_rear_2d_constraint = DircolPositionData<double>(plant, toe_left,
        pt_rear_contact, isXZ, Eigen::Vector2d::Zero(), isYZ);
  auto right_toe_rear_2d_constraint = DircolPositionData<double>(plant, toe_right,
        pt_rear_contact, isXZ, Eigen::Vector2d::Zero(), isYZ);
  ...
  auto left_toe_mid_constraint = DircolPositionData<double>(plant, toe_left,
                                 pt_mid_contact, isXZ);
  auto right_toe_mid_constraint = DircolPositionData<double>(plant, toe_right,
                                  pt_mid_contact, isXZ);
  ...
  vector<DircolKinematicData<double>*> left_stance_ht_constraint;
  ...
  auto left_ht_dataset = DircolKinematicDataSet<double>(plant,
                      &left_stance_ht_constraint);
  auto left_ht_options = DircolOptions(left_ht_dataset.countConstraints());
  ...
  vector<DircolKinematicData<double>*> left_stance_t_constraint;
  ...
  auto left_t_dataset = DircolKinematicDataSet<double>(plant,
                      &left_stance_t_constraint);
  auto left_t_options = DircolOptions(left_t_dataset.countConstraints());
  left_t_options.setConstraintRelative(0, true);
  left_t_options.setConstraintRelative(1, true);

  // Double stance (left toe, right heel)
  vector<DircolKinematicData<double>*> double_stance_th_constraint;
  ...
  auto double_th_dataset = DircolKinematicDataSet<double>(plant,
                        &double_stance_th_constraint);
  auto double_th_options = DircolOptions(double_th_dataset.countConstraints());
  ...
  vector<DircolKinematicData<double>*> double_stance_tht_constraint;
  ...
  auto double_tht_dataset = DircolKinematicDataSet<double>(plant,
                        &double_stance_tht_constraint);
  auto double_tht_options = DircolOptions(double_tht_dataset.countConstraints());
  ...
  vector<DircolKinematicData<double>*> right_stance_ht_constraint;
  ...
  auto right_ht_dataset = DircolKinematicDataSet<double>(plant,
                       &right_stance_ht_constraint);
  auto right_ht_options = DircolOptions(right_ht_dataset.countConstraints());
  ...
  vector<DircolKinematicData<double>*> double_stance_all_constraint;
  ...
  auto double_all_dataset = DircolKinematicDataSet<double>(plant,
                        &double_stance_all_constraint);
  auto double_all_options = DircolOptions(double_all_dataset.countConstraints());
  ...
  // Testing
  vector<DircolKinematicData<double>*> double_stance_all_2d_constraint;
  ...
  auto double_all_2d_dataset = DircolKinematicDataSet<double>(plant,
                        &double_stance_all_2d_constraint);
  auto double_all_2d_options = DircolOptions(double_all_2d_dataset.countConstraints());
  if (set_both_contact_pos_manually) {
  ...
  vector<DircolKinematicData<double>*> left_stance_mid_constraint;
  ...
  auto left_mid_dataset = DircolKinematicDataSet<double>(plant,
                      &left_stance_mid_constraint);
  auto left_mid_options = DircolOptions(left_mid_dataset.countConstraints());
  ...
  vector<DircolKinematicData<double>*> right_stance_mid_constraint;
  ...
  auto right_mid_dataset = DircolKinematicDataSet<double>(plant,
                       &right_stance_mid_constraint);
  auto right_mid_options = DircolOptions(right_mid_dataset.countConstraints());
  ...
  vector<DircolKinematicData<double>*> double_stance_mid_constraint;
  ...
  auto double_mid_dataset = DircolKinematicDataSet<double>(plant,
                        &double_stance_mid_constraint);
  auto double_mid_options = DircolOptions(double_mid_dataset.countConstraints());
  ...
  vector<DircolKinematicDataSet<double>*> dataset_list;
  vector<DircolOptions> options_list;
  if (walking_mode == 2) {
  ...
  min_dt.push_back(.01);
  max_dt.push_back(.3);
  dataset_list.push_back(&no_stance_dataset);
  options_list.push_back(no_stance_options);
  ...
  auto trajopt = std::make_shared<HybridDircol<double>>(plant,
  ...
  //   for (int index = 0; index < num_time_samples[mode]-1; index++) {
  //     auto xcol_i = trajopt->x_col(mode, index);
  //     // trajopt->AddLinearConstraint(xcol_i(positions_map.at("position[4]")) == -dist/2);
  //     // trajopt->AddLinearConstraint(xcol_i(positions_map.at("position[5]")) == 0);
  //     // trajopt->AddLinearConstraint(xcol_i(positions_map.at("position[6]")) == 1);
  //     trajopt->AddLinearConstraint(xcol_i(n_q + velocities_map.at("velocity[3]")) == 0);
  //     trajopt->AddLinearConstraint(xcol_i(n_q + velocities_map.at("velocity[4]")) == 0);
  //     trajopt->AddLinearConstraint(xcol_i(n_q + velocities_map.at("velocity[5]")) == 0);
  //     trajopt->AddLinearConstraint(xcol_i(n_q + velocities_map.at("velocity[0]")) == 0);
  //     trajopt->AddLinearConstraint(xcol_i(n_q + velocities_map.at("velocity[1]")) == 0);
  //     trajopt->AddLinearConstraint(xcol_i(n_q + velocities_map.at("velocity[2]")) == 0);
  //   }
  // }
  ...
  for (const auto & member : joint_names) {
    ...
    for (unsigned int mode = 0; mode < num_time_samples.size(); mode++) {
      for (int index = 0; index < num_time_samples[mode]-1; index++) {
        auto xcol_i = trajopt->x_col(mode, index);
        trajopt->AddLinearConstraint(
          xcol_i(positions_map.at(member)) <=
          plant.GetJointByName(member).position_upper_limits()(0));
        trajopt->AddLinearConstraint(
          xcol_i(positions_map.at(member)) >=
          plant.GetJointByName(member).position_lower_limits()(0));
  ...
  for (const auto & member : motor_names) {
    ...
    for (unsigned int mode = 0; mode < num_time_samples.size(); mode++) {
      for (int index = 0; index < num_time_samples[mode]-1; index++) {
        auto ucol_i = trajopt->u_col(mode, index);
        trajopt->AddLinearConstraint(ucol_i(actuators_map.at(member)) <= 300);
        trajopt->AddLinearConstraint(ucol_i(actuators_map.at(member)) >= -300);
      }
    }
  }
  ...
  MatrixXd R = 10 * MatrixXd::Identity(n_u, n_u);
  ...
  for (unsigned int mode = 0; mode < num_time_samples.size(); mode++) {
    for (int index = 0; index < num_time_samples[mode]-1; index++) {
      auto xcol_i = trajopt->x_col(mode, index);
      auto ucol_i = trajopt->u_col(mode, index);
      // trajopt->AddQuadraticCost(Q, VectorXd::Zero(n_v), xcol_i.tail(n_v));
      trajopt->AddQuadraticCost(R, VectorXd::Zero(n_u), ucol_i);
    }
  }
  ...
  if (!init_file.empty()) {
    ...
    if (standing) {
      ...
      //     if (index != num_time_samples[mode]-1) {
      //       auto xcol_i = trajopt->x_col(mode, index);
      //       trajopt->SetInitialGuess(xcol_i, xi_init);
      //       auto ucol_i = trajopt->u_col(mode, index);
      ...
      for (unsigned int mode = 0; mode < num_time_samples.size(); mode++) {
        for (int index = 0; index < num_time_samples[mode]; index++) {
          ...
          VectorXd xi_init(n_q + n_v);
          if (i < N_ss) {
            xi_init << q_seed.at(i), v_seed.at(i);
          } else {
            xi_init << q_seed.at(N_ss - 1), v_seed.at(N_ss - 1);
          }
          trajopt->SetInitialGuess(xi, xi_init);
      ...
      vector<VectorXd> u_init(N, VectorXd::Zero(n_u));
      vector<VectorXd> lambda_init(N, VectorXd::Zero(
      ...
      for (int i = 0; i < N; i++) {
        auto ui = trajopt->input(i);
        trajopt->SetInitialGuess(ui, u_init.at(i));

        // trajopt->SetInitialGuess(lambdai, lambda_init.at(i));
      }
      */
    }
  }
  ...
  for (unsigned int mode = 0; mode < num_time_samples.size(); mode++) {
    for (int index = 0; index < num_time_samples[mode]; index++) {
      ...
      if (index != num_time_samples[mode]-1) {
        auto xcol_i = trajopt->x_col(mode, index);
        if ((trajopt->GetInitialGuess(xcol_i.head(4)).norm() == 0) ||
            std::isnan(trajopt->GetInitialGuess(xcol_i.head(4)).norm())) {
          trajopt->SetInitialGuess(xcol_i(0), 1);
          trajopt->SetInitialGuess(xcol_i(1), 0);
          trajopt->SetInitialGuess(xcol_i(2), 0);
          trajopt->SetInitialGuess(xcol_i(3), 0);
        }
      }
    }
    i_start += num_time_samples[mode];
