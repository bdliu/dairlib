...
DynamicsConstraint::DynamicsConstraint(
  ...
  DRAKE_DEMAND((n_zDDot/2) * n_featureDot == thetaZDDot.size());
...
void DynamicsConstraint::DoEval(const
  ...
  // AutoDiffVecXd h_of_z_i = expression_object_.getExpression(thetaZDDot_,
  //     z_i);
  // AutoDiffVecXd h_of_z_iplus1 = expression_object_.getExpression(thetaZDDot_,
  //     z_iplus1);
  // AutoDiffVecXd h_of_colloc_pt = expression_object_.getExpression(thetaZDDot_,
  //     z_collocation);
...
VectorXd DynamicsConstraint::getGradientWrtTheta(
  ...
  for(int i = 0; i<n_featureZDDot_; i++){
    ...
    theta_unit(i) = 1;
    gradient(i) = getDynamicsConstraint(z_i,z_iplus1,timestep_i,theta_unit)(n_zDDot_/2) - z_iplus1(n_zDDot_/2) + z_i(n_zDDot_/2);
  }
