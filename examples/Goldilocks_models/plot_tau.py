...
for iteration in range(iteration_start,iteration_end+1):
    if os.path.isfile(directory+str(iteration)+'_'+str(batch)+'_t_and_tau.csv'):
        matrix = np.genfromtxt (directory+str(iteration)+'_'+str(batch)+'_t_and_tau.csv', delimiter=",")
        n_rows = (matrix.shape)[0]

        for index in range(1,n_rows):
            tau_i_th_element = matrix[index,:]
            max_list.append(np.amax(tau_i_th_element))
            min_list.append(np.amin(tau_i_th_element))
...
for iteration in range(iteration_start,iteration_end+1):
    ...
    if os.path.isfile(directory+str(iteration)+'_'+str(batch)+'_t_and_tau.csv'):
        ...
        for index in range(1,n_rows):
            t = matrix[0,:]
            tau_i_th_element = matrix[index,:]
            ax.plot(t,tau_i_th_element, label='tau_'+str(index))
