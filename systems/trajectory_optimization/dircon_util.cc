  ...
  for (auto const& binding : prog->GetAllCosts()) {
    ...
    for (int i = 0; i < variables.size(); i++) {
      ...
      for (int j=0; j <= i; j++) {
        int ind_i = prog->FindDecisionVariableIndex(variables(i));
        int ind_j = prog->FindDecisionVariableIndex(variables(j));
        Q(ind_i, ind_j) += (gradient_hessian_p(0, j) - gradient_hessian_m(0, j)) / dx;
        if (ind_i != ind_j)
          Q(ind_j, ind_i) += (gradient_hessian_p(0, j) - gradient_hessian_m(0, j))/dx;
      }
    }
...
void linearizeConstraints(const MathematicalProgram* prog, const VectorXd& x,
  ...
  if (is_old_version) {
    ...
    constraint_index = updateConstraints(prog, prog->bounding_box_constraints(), x,
                                         y, A, lb, ub, constraint_index);
    constraint_index = updateConstraints(prog, prog->linear_constraints(), x, y, A,
                                         lb, ub, constraint_index);
    constraint_index = updateConstraints(prog, prog->linear_equality_constraints(),
                                         x, y, A, lb, ub, constraint_index);
    constraint_index = updateConstraints(prog, prog->lorentz_cone_constraints(), x,
                                         y, A, lb, ub, constraint_index);
    constraint_index = updateConstraints(prog, prog->generic_constraints(), x, y, A,
                                         lb, ub, constraint_index);
  }
  else {
    int num_constraints = 0;
...
void newlinearizeConstraints(const MathematicalProgram* prog, const VectorXd& x,
                          VectorXd& y, MatrixXd& A, VectorXd& lb, VectorXd& ub) {
  int num_constraints = 0;
  int num_vars = prog->num_vars();
...
int updateConstraints(const MathematicalProgram* prog,
                      const std::vector<Binding<Derived>>& constraints,
                      const VectorXd& x, VectorXd& y, MatrixXd& A, VectorXd& lb, VectorXd& ub,
