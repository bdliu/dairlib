...
/// An implementation of DircolKinematicData to address distance constraints,
/// which specify the distance between two given points on two bodies
/// computes the constraint as distance(q)^2 - d^2 = 0
template <typename T>
class DircolDistanceData : public DircolKinematicData<T> {
 public:
  DircolDistanceData(const drake::multibody::MultibodyPlant<T>& plant,
