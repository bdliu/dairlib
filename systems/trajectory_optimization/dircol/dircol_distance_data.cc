...
template <typename T>
DircolDistanceData<T>::DircolDistanceData(const MultibodyPlant<T>& plant,
      const Body<T>& body1, const Vector3d pt1, const Body<T>& body2,
      const Vector3d pt2, const double distance) :
    DircolKinematicData<T>(plant, 1),
    body1_(body1),
    body2_(body2),
    pt1_(pt1),
    pt2_(pt2),
...
template <typename T>
DircolDistanceData<T>::~DircolDistanceData() {
}

template <typename T>
void DircolDistanceData<T>::updateConstraint(const Context<T>& context) {
...
template class DircolDistanceData<double>;
template class DircolDistanceData<drake::AutoDiffXd>;
}  // namespace dairlib
