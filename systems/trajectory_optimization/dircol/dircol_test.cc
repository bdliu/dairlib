...
#include "dircol_position_data.h"
#include "dircol_kinematic_data_set.h"
#include "dircol.h"
...
using std::cout;
using std::endl;
using drake::systems::trajectory_optimization::Dircol;
using drake::systems::trajectory_optimization::HybridDircol;
using drake::systems::trajectory_optimization::DirectCollocation;
using drake::systems::trajectory_optimization::DircolDynamicConstraint;
using drake::systems::trajectory_optimization::DircolKinematicConstraint;
using drake::systems::trajectory_optimization::DircolOptions;
using drake::systems::trajectory_optimization::DircolKinConstraintType;
...
int testConstraints() {
  ...
  pt2 << 0,0,0;
  bool isXZ = true;
  DircolPositionData<AutoDiffXd> constraintd = DircolPositionData<AutoDiffXd>(tree,bodyIdx,pt,isXZ);
  DircolPositionData<AutoDiffXd> constraintd2 = DircolPositionData<AutoDiffXd>(tree,bodyIdx,pt2,isXZ);
  DircolPositionData<double> constraint = DircolPositionData<double>(tree,bodyIdx,pt,isXZ);
  DircolPositionData<double> constraint2 = DircolPositionData<double>(tree,bodyIdx,pt2,isXZ);
  cout << "??" <<endl;
  int n = 4;
  int nl = 4;
  int nu = 1;
  ...
  std::vector<DircolKinematicData<AutoDiffXd>*> constraintsd;
  constraintsd.push_back(&constraintd);
  constraintsd.push_back(&constraintd2);
  auto datasetd = DircolKinematicDataSet<AutoDiffXd>(tree, &constraintsd);

  //auto dataset_ptr =  std::unique_ptr<DircolKinematicDataSet<AutoDiffXd>>(&datasetd);

   datasetd.updateData(x_autodiff, u_autodiff, l_autodiff);

  auto dynamicConstraint = std::make_shared<DircolDynamicConstraint<AutoDiffXd>>(tree, datasetd);
  auto kinematicConstraint = std::make_shared<DircolKinematicConstraint<AutoDiffXd>>(tree, datasetd);
...
template <typename T>
int testDircol(bool addForceConstraints, Eigen::VectorXd x0 = Eigen::VectorXd::Zero(8)) {
  RigidBodyTree<double> tree;
  parsers::urdf::AddModelInstanceFromUrdfFileToWorld("../../examples/Acrobot/Acrobot_floating.urdf", multibody::joints::kFixed, &tree);
  //const std::unique_ptr<const RigidBodyTree<double>> tree =  std::unique_ptr<const RigidBodyTree<double>>(&model);
  ...
  pt << 0,0,0;
  bool isXZ = true;
  auto constraint = DircolPositionData<T>(tree,bodyIdx,pt,isXZ);
  ...
  std::vector<DircolKinematicData<T>*> constraints;
  constraints.push_back(&constraint);
  auto dataset = DircolKinematicDataSet<T>(tree, &constraints);

  int N = 11;
  auto options = DircolOptions(dataset.countConstraints());
  options.setStartType(DircolKinConstraintType::kAccelOnly);
  options.setEndType(DircolKinConstraintType::kAccelOnly);
  //options.setConstraintRelative(0,true);
  auto trajopt = std::make_shared<Dircol<T>>(tree, N, .02, .3, dataset, options);
...
template <typename T>
int testHybridDircol(bool addForceConstraints, Eigen::VectorXd x0 = Eigen::VectorXd::Zero(8)) {
  RigidBodyTree<double> tree;
  parsers::urdf::AddModelInstanceFromUrdfFileToWorld("../../examples/Acrobot/Acrobot_floating.urdf", multibody::joints::kFixed, &tree);
  //const std::unique_ptr<const RigidBodyTree<double>> tree =  std::unique_ptr<const RigidBodyTree<double>>(&model);
  ...
  max_dt.push_back(.3);
  std::vector<DircolKinematicDataSet<T>*> dataset_list;
  dataset_list.push_back(&dataset);

  std::vector<DircolOptions> options_list;
  options_list.push_back(options);
  
  auto trajopt = std::make_shared<HybridDircol<T>>(tree, timesteps, min_dt, max_dt, dataset_list, options_list);

  // HybridDircol(const RigidBodyTree<double>& tree, vector<int> num_time_samples, vector<double> minimum_timestep,
    // vector<double> maximum_timestep, vector<DircolKinematicDataSet<T>*> constraints, vector<DircolOptions> options);
...
template <typename T>
int testHybridDircolJump(bool addForceConstraints, Eigen::VectorXd x0 = Eigen::VectorXd::Zero(8)) {
  RigidBodyTree<double> tree;
  parsers::urdf::AddModelInstanceFromUrdfFileToWorld("../../examples/Acrobot/Acrobot_floating.urdf", multibody::joints::kFixed, &tree);
  //const std::unique_ptr<const RigidBodyTree<double>> tree =  std::unique_ptr<const RigidBodyTree<double>>(&model);
  ...
  std::vector<DircolKinematicData<T>*> no_constraints;
  auto dataset_free = DircolKinematicDataSet<T>(tree, &no_constraints);

  auto options = DircolOptions(dataset.countConstraints());
  options.setStartType(DircolKinConstraintType::kAccelOnly);
  auto options_end = DircolOptions(dataset.countConstraints());
  options_end.setEndType(DircolKinConstraintType::kAll);
  ...
  std::vector<DircolOptions> options_list;
  options_list.push_back(options);
  options_list.push_back(DircolOptions(0));
...
RigidBodyTree<double> tree;
  ...
  DircolPositionData<AutoDiffXd> constraint = DircolPositionData<AutoDiffXd>(tree,bodyIdx,pt,isXZ);
  std::vector<DircolKinematicData<AutoDiffXd>*> constraints;
  constraints.push_back(&constraint);
  auto dataset = DircolKinematicDataSet<AutoDiffXd>(tree, &constraints);
  auto dyn_constraint = std::make_shared<DircolDynamicConstraint<AutoDiffXd>>(tree, dataset);

  DircolPositionData<double> constraint_double = DircolPositionData<double>(tree,bodyIdx,pt,isXZ);
  std::vector<DircolKinematicData<double>*> constraints_double;
  constraints_double.push_back(&constraint_double);
  auto dataset_double = DircolKinematicDataSet<double>(tree, &constraints_double);
  auto dyn_constraint_double = std::make_shared<DircolDynamicConstraint<double>>(tree, dataset_double);
...
int testDircol() {
  RigidBodyTree<double> tree_dircol;
  parsers::urdf::AddModelInstanceFromUrdfFileToWorld("../../examples/Acrobot/Acrobot.urdf", multibody::joints::kFixed, &tree_dircol);
  ...
  // std::vector<DircolKinematicData<AutoDiffXd>*> constraints;
  // auto dataset = DircolKinematicDataSet<AutoDiffXd>(tree_dircol, &constraints);
  // auto options = DircolOptions(dataset.getNumConstraints());
  // auto trajopt = std::make_shared<Dircol>(tree_dircol, N, .01, 3.0, dataset, options);
...
int main(int argc, char* argv[]) {
  ...
  switch(FLAGS_testIndex) {
    case 0:
      ...
      drake::dircol::examples::testDircol<drake::AutoDiffXd>(false);
      break;
    case 1:
    std::cout << "Testing DIRCOL with double" << std::endl;
      drake::dircol::examples::testDircol<double>(false);
      break;
    case 2:
      std::cout << "Testing DIRCOL with force constraints (autodiff)" << std::endl;
      drake::dircol::examples::testDircol<drake::AutoDiffXd>(true, init_up);
      break;
    case 3:
      std::cout << "Testing DIRCOL with force constraints (double)" << std::endl;
      drake::dircol::examples::testDircol<double>(true, init_up);
      break;
    case 4:
    std::cout << "Testing Dircol with AutoDiffXd" << std::endl;
      drake::dircol::examples::testDircol();
      break;
    case 5:
    std::cout << "Testing DIRCOL optimization constraints" << std::endl;
      drake::dircol::examples::testDircolConstraints();
      break;
    case 6:
    std::cout << "Testing kinematic constraints" << std::endl;
      drake::dircol::examples::testConstraints();
      break;
    case 7:
      std::cout << "Testing hybrid DIRCOL (Acrobot, one mode) with double" << std::endl;
      drake::dircol::examples::testHybridDircol<double>(false);
      break;
    case 8:
      Eigen::VectorXd init_jump = Eigen::VectorXd(8);
      init_jump << 0, 0, M_PI*3/4, M_PI/2, 0, 0, 0, 0;
      std::cout << "Testing hybrid DIRCOL (Acrobot, three modes jumping then swingup) with double" << std::endl;
      drake::dircol::examples::testHybridDircolJump<double>(true);
      break;
  }
  return 0;
}
