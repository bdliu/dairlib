...
template <typename T>
class DircolKinematicDataSet {
 public:
  DircolKinematicDataSet(const drake::multibody::MultibodyPlant<T>& plant,
  ...
  drake::VectorX<T> getQDot();
  // drake::VectorX<T> getVDot();
  // drake::VectorX<T> getXDot();
  drake::MatrixX<T> getM();
  drake::VectorX<T> getRightHandSide();

  DircolKinematicData<T>* getConstraint(int index);
 ...
 private:
  ...
  std::vector<DircolKinematicData<T>*>* constraints_;
  int num_positions_;
  int num_velocities_;
