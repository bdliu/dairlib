...
template <typename T>
DircolPositionData<T>::DircolPositionData(const MultibodyPlant<T>& plant,
    const Body<T>& body, Vector3d pt, bool isXZ, Vector2d ground_rp,
    bool isYZ) :
    DircolKinematicData<T>(plant, isXZ ? 2 : 3),
    body_(body),
    pt_(pt),
    isXZ_(isXZ) {
  TXZ_ << 1, 0, 0,
...
template <typename T>
DircolPositionData<T>::~DircolPositionData() {
}

template <typename T>
void DircolPositionData<T>::updateConstraint(const Context<T>& context) {
...
template <typename T>
void DircolPositionData<T>::addFixedNormalFrictionConstraints(Vector3d normal,
                                                              double mu) {
  if (isXZ_) {
...
template class DircolPositionData<double>;
template class DircolPositionData<drake::AutoDiffXd>;
}  // namespace dairlib
